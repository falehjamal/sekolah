    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header flex-wrap gap-3 d-flex justify-content-between align-items-center">
                <div>
                    <h5 class="mb-1">Data Cuti Guru</h5>
                    <p class="text-muted mb-0 small">Kelola pengajuan cuti guru dan approval.</p>
                </div>
                <button type="button" class="btn btn-primary" onclick="tambahData()">
                    <i class="bx bx-plus me-1"></i> Tambah Cuti
                </button>
            </div>
            <div class="card-body">
                <div class="table-responsive text-nowrap">
                    <table class="table table-striped table-modern" id="tableCuti">
                        <thead>
                            <tr>
                                <th width="5%">No</th>
                                <th>Guru</th>
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalFormTitle">Tambah Cuti Guru</h5>
            ...
            <form id="formCuti">
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="jenis_cuti" name="jenis_cuti" placeholder="Contoh: Cuti Tahunan, Cuti Sakit, Cuti Melahirkan" required>
                        <div class="invalid-feedback"></div>
                    </div>
...
<div class="modal fade" id="modalDetail" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Detail Cuti Guru</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                ...
                <hr>
                <h6 class="mb-3"><i class="bx bx-calendar me-1"></i> Informasi Cuti</h6>

                <div class="detail-row">
                    <span class="detail-label">Jenis Cuti</span>
                    <span class="detail-value" id="detail_jenis_cuti">-</span>
                </div>
                <div class="detail-row">
                    <span class="detail-label">Tanggal Awal</span>
                    <span class="detail-value" id="detail_tanggal_awal">-</span>
                </div>
                <div class="detail-row">
                    <span class="detail-label">Tanggal Akhir</span>
                    <span class="detail-value" id="detail_tanggal_akhir">-</span>
                </div>
                <div class="detail-row">
                    <span class="detail-label">Durasi</span>
                    <span class="detail-value" id="detail_durasi">-</span>
                </div>
                <div class="detail-row">
                    <span class="detail-label">Waktu Entry</span>
                    <span class="detail-value" id="detail_waktu_entry">-</span>
                </div>
                <div class="detail-row" id="approval_info" style="display: none;">
                    <span class="detail-label">Disetujui oleh</span>
                    <span class="detail-value" id="detail_petugas">-</span>
                </div>
                <div class="detail-row" id="approval_time" style="display: none;">
                    <span class="detail-label">Waktu Approval</span>
                    <span class="detail-value" id="detail_waktu_approval">-</span>
                </div>
            </div>
            <div class="modal-footer">
...
$(document).ready(function() {
    ...
    table = $('#tableCuti').DataTable({
        ...
        columns: [
            ...
            { data: 'jenis_cuti_display', name: 'jenis_cuti', orderable: false, searchable: false },
            { data: 'periode_display', name: 'tanggal_awal', orderable: false, searchable: false },
            { data: 'waktu_entry_display', name: 'waktu_entry', orderable: false, searchable: false },
            { data: 'status_badge', name: 'status_approval', orderable: false, searchable: false },
...
function tambahData() {
    isEditMode = false;
    $('#modalFormTitle').text('Tambah Cuti Guru');
    $('#formCuti')[0].reset();
    $('#cuti_id').val('');
    $('#guru_id').val('').trigger('change');
    $('#status_approval').val('pending');
    clearValidation();
...
function editData(id) {
    isEditMode = true;
    $('#modalFormTitle').text('Edit Cuti Guru');
    clearValidation();
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response.success) {
                ...
                $('#status_approval').val(data.status_approval);
        ...
        error: function(xhr) {
            hideLoading();
            showToast('error', 'Error', 'Gagal mengambil data cuti');
        }
    });
}
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response.success) {
                ...
                $('#detail_status').html(data.status_approval_badge);

                // Cuti details
                $('#detail_jenis_cuti').text(data.jenis_cuti);
                $('#detail_tanggal_awal').text(data.tanggal_awal_format);
                $('#detail_tanggal_akhir').text(data.tanggal_akhir_format);
                $('#detail_durasi').text(data.durasi_hari + ' hari');
                $('#detail_waktu_entry').text(data.waktu_entry);

                // Approval info
                if (data.status_approval !== 'pending' && data.petugas_nama !== '-') {
                    $('#detail_petugas').text(data.petugas_nama);
                    $('#detail_waktu_approval').text(data.waktu_approval);
                    $('#approval_info').show();
                    $('#approval_time').show();
                } else {
                    $('#approval_info').hide();
...
function simpanData() {
    const cutiId = $('#cuti_id').val();
    const url = cutiId ? "{{ url('cuti') }}/" + cutiId : "{{ route('cuti.store') }}";
    const method = cutiId ? 'PUT' : 'POST';
    ...
    const formData = {
        ...
        jabatan_guru: $('#jabatan_guru').val() || null,
        status_approval: $('#status_approval').val()
    };
...
function deleteData(id) {
    if (confirm('Apakah Anda yakin ingin menghapus data cuti ini?')) {
        ...
        $.ajax({
            ...
            error: function(xhr) {
                hideLoading();
                showToast('error', 'Error', 'Gagal menghapus data cuti');
            }
        });
    }
}
...
function approveData(id) {
    if (confirm('Apakah Anda yakin ingin approve cuti ini?')) {
        showLoading();

        $.ajax({
            url: "{{ url('cuti') }}/" + id + "/approve",
            type: 'POST',
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            ...
            error: function(xhr) {
                hideLoading();
                showToast('error', 'Error', 'Gagal approve cuti');
            }
        });
    }
}
...
function rejectData(id) {
    if (confirm('Apakah Anda yakin ingin reject cuti ini?')) {
        showLoading();

        $.ajax({
            url: "{{ url('cuti') }}/" + id + "/reject",
            type: 'POST',
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            ...
            error: function(xhr) {
                hideLoading();
                showToast('error', 'Error', 'Gagal reject cuti');
            }
        });
    }
}
