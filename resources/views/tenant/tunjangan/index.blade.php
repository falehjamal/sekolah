    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header flex-wrap gap-3 d-flex justify-content-between align-items-center">
                <div>
                    <h5 class="mb-1">Data Tunjangan Guru</h5>
                    <p class="text-muted mb-0 small">Kelola data tunjangan guru.</p>
                </div>
                <button type="button" class="btn btn-primary" onclick="tambahData()">
                    <i class="bx bx-plus me-1"></i> Tambah Tunjangan
                </button>
            </div>
            <div class="card-body">
                <div class="table-responsive text-nowrap">
                    <table class="table table-striped table-modern" id="tableTunjangan">
                        <thead>
                            <tr>
                                <th width="5%">No</th>
                                <th>Guru</th>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalFormTitle">Tambah Tunjangan Guru</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form id="formTunjangan">
                <div class="modal-body">
                    <input type="hidden" id="tunjangan_id" name="tunjangan_id">
                    ...
                    <div class="mb-3">
                        <label for="nama_tunjangan" class="form-label">Nama Tunjangan <span class="text-danger">*</span></label>
                        <input type="text" class="form-control" id="nama_tunjangan" name="nama_tunjangan" placeholder="Contoh: Tunjangan Sertifikasi" required>
                        <div class="invalid-feedback"></div>
                    </div>

                    <div class="mb-3">
                        <label for="nominal_tunjangan" class="form-label">Nominal Tunjangan <span class="text-danger">*</span></label>
                        <div class="input-group">
                            <span class="input-group-text">Rp</span>
                            <input type="text" class="form-control" id="nominal_tunjangan" name="nominal_tunjangan" placeholder="0" required>
                        </div>
                        <div class="invalid-feedback"></div>
                    </div>
...
<div class="modal fade" id="modalDetail" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Detail Tunjangan Guru</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <!-- Guru Info -->
                ...
                <hr>
                <h6 class="mb-3"><i class="bx bx-money me-1"></i> Informasi Tunjangan</h6>

                <div class="detail-row">
                    <span class="detail-label">Nama Tunjangan</span>
                    <span class="detail-value" id="detail_nama_tunjangan">-</span>
                </div>
                <div class="detail-row">
                    <span class="detail-label">Nominal Tunjangan</span>
                    <span class="detail-value" id="detail_nominal_tunjangan">-</span>
                </div>
                <div class="detail-row">
...
$(document).ready(function() {
    ...
    $('#nominal_tunjangan').on('input', function() {
        const value = $(this).val().replace(/[^0-9.,]/g, '');
        $(this).val(value);
    });

    table = $('#tableTunjangan').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
        ...
        columns: [
            ...
            { data: 'nama_tunjangan', name: 'nama_tunjangan' },
            { data: 'nominal_tunjangan_display', name: 'nominal_tunjangan', orderable: false, searchable: false, className: 'text-nowrap' },
...
function tambahData() {
    isEditMode = false;
    $('#modalFormTitle').text('Tambah Tunjangan Guru');
    $('#formTunjangan')[0].reset();
    $('#guru_id').val('').trigger('change');
    $('#tunjangan_id').val('');
...
function editData(id) {
    ...
    $.ajax({
        ...
        error: function() {
            hideLoading();
            showToast('error', 'Error', 'Gagal mengambil data tunjangan');
        }
    });
}
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response.success) {
                ...
                $('#detail_nama_tunjangan').text(data.nama_tunjangan || '-');
                $('#detail_nominal_tunjangan').text(data.nominal_tunjangan_format || '-');
...
function simpanData() {
    const id = $('#tunjangan_id').val();
    const url = id ? "{{ url('tunjangan') }}/" + id : "{{ route('tunjangan.store') }}";
    const method = id ? 'PUT' : 'POST';

    const formData = {
        guru_id: $('#guru_id').val(),
        nama_tunjangan: $('#nama_tunjangan').val(),
        nominal_tunjangan: $('#nominal_tunjangan').val(),
        waktu: $('#waktu').val()
    };
...
function deleteData(id) {
    if (confirm('Apakah Anda yakin ingin menghapus data tunjangan ini?')) {
        showLoading();
        ...
        $.ajax({
            ...
            error: function() {
                hideLoading();
                showToast('error', 'Error', 'Gagal menghapus data tunjangan');
            }
        });
    }
}
