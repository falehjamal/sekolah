...
class UpdateSiswaRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $ignoreId = $this->currentSiswaId();

        return [
            'user_id' => $this->userRule($ignoreId),
            'nis' => 'required|string|max:20|'.$this->uniqueRule($siswaTable, 'nis', $ignoreId),
            'nisn' => 'required|string|max:20|'.$this->uniqueRule($siswaTable, 'nisn', $ignoreId),
    ...
    protected function currentSiswaId(): ?int
    {
        $routeParam = $this->route('siswa');

        if ($routeParam instanceof Siswa) {
